...
public class Camellia {
    ...
    private static final String TAG = Camellia.class.getSimpleName();

    final static private int[][] SBOX1 = {{112, 130, 44, 236, 179, 39, 192, 229, 228, 133, 87, 53, 234, 12, 174, 65},
    ...
    final static private long[] SIGMA = {0xA09E667F3BCC908BL, 0xB67AE8584CAA73B2L, 0xC6EF372FE94F82BEL,
            0x54FF53A5F1D36F1CL, 0x10E527FADE682D1DL, 0xB05688C2B3E6C1FDL};

    private static byte cycleShift(byte value, int count) {
        int temp = (value & 0xFF) >> (8 - count);
        return (byte) ((value << count) + temp);
    }

    private static int cycleShift(int value) {
        int temp = (value) >>> (32 - 1);
        return ((value << 1) + temp);
    }

    @NotNull
    private static long[] cycleShiftForPair(@NotNull long[] values, int count) {
    ...
    private static long FFunc(long data, long subkey) {
    ...
    private static long[] getKLKR(String string_key) {
        long[] key = new long[4];
    ...
    private static long[] getKAKB(long[] KL_KR) {
    ...
    private static long[] getSubkeys128(long[] KL_KR, long[] KA_KB) {
    ...
    private static long[] getSubkeys192_256(long[] KL_KR, long[] KA_KB) {
    ...
    public static long[] keySchedule(String string_key) {
        long[] KL_KR = getKLKR(string_key);
        long[] KA_KB = getKAKB(KL_KR);
        return (string_key.length() <= 16) ? getSubkeys128(KL_KR, KA_KB) : getSubkeys192_256(KL_KR, KA_KB);
    }

    protected long[] keySchedule(byte[] byte_key) {
        long[] KL_KR = getKLKRByte(byte_key);
        long[] KA_KB = getKAKB(KL_KR);
        return (byte_key.length <= 16) ? getSubkeys128(KL_KR, KA_KB) : getSubkeys192_256(KL_KR, KA_KB);
    }

    private static long FLFunc(long data, long subkey) {
        int x1, x2, k1, k2;
        x1 = (int) (data >>> 32);
        x2 = (int) (data & 0xFFFFFFFFL);
        k1 = (int) (subkey >>> 32);
        k2 = (int) (subkey & 0xFFFFFFFFL);
        x2 = x2 ^ (cycleShift((x1 & k1)));
        x1 = x1 ^ (x2 | k2);
        return ((long) x1 << 32) | (long) x2 & 0xFFFFFFFFL;
    }

    private static long FLINVFunc(long data, long subkey) {
        int y1, y2, k1, k2;
        y1 = (int) (data >>> 32);
        ...
        k2 = (int) (subkey & 0xFFFFFFFFL);
        y1 = y1 ^ (y2 | k2);
        y2 = y2 ^ (cycleShift((y1 & k1)));
        return ((long) y1 << 32) | (long) y2 & 0xFFFFFFFFL;
    }

    public static long[] crypt(long D1, long D2, long[] subkeys) {
